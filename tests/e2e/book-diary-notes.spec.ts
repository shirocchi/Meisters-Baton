...
test('diary notes keep source dates and editorial cautions visible in the relevant chapters', async ({
  ...
  await expect(page.locator('.book-diary-card')).toHaveCount(13);
  await page.goto('/#library/textbook/flange/0');
  const note = page.locator('.book-diary-card');
  ...
  await page.getByRole('button', { name: 'この章の付箋 1' }).click();
  ...
  await expect(page.locator('.book-diary')).toContainText('試験を始めません');
